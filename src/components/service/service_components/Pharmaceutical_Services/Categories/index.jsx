...
const CategoriesPharma = () => {
  ...
  const onNavigateSearch = ({ category }) => {
    navigate(`/search?q=${category?.name}`, { state: { selectedCardId: selectedServiceId, categoryId: category?._id } })
  }
      ...
      {isLoading ?
        ...
        <div className="flex md:flex-row flex-col gap-14 mt-8 px-4 justify-center items-center">
          {pharmaCategories && pharmaCategories?.length > 0 && pharmaCategories.map((category) => (
            <div onClick={() => onNavigateSearch({ category })} className="w-full md:w-2/4" key={category?._id}>
