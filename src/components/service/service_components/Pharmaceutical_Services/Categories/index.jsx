...
const CategoriesPharma = () => {
  ...
  const onNavigateSearch = ({ category }) => {
    navigate(`/search?q=${category?.name}`, {
      state: { selectedCardId: selectedServiceId, categoryId: category?._id },
    });
  };
    ...
    <div className="mt-16 flex flex-col items-center">
      ...
      {isLoading ? (
        ...
        <div className="flex md:flex-row flex-col gap-14 mt-8 px-4 justify-center items-center">
          {pharmaCategories &&
            pharmaCategories?.length > 0 &&
            pharmaCategories.map((category) => (
              <div
                onClick={() => onNavigateSearch({ category })}
